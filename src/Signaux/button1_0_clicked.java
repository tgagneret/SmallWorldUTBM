...
public class button1_0_clicked implements ActionListener { 
	...
	public button1_0_clicked(MainWindow fenetre){
	fen = fenetre;
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(fen.get_mode()){
			if(Map.getInstance().get_case(1, 0).get_joueur() == null ||
					Map.getInstance().get_case(1, 0).get_joueur().get_name()
			...
			JOptionPane.showMessageDialog(fen,
				...
				Organize organise = new Organize(fen,1,0);
			}
			...
			if(Map.getInstance().get_case(1, 0).get_joueur() == null || 
					Map.getInstance().get_case(1, 0).get_joueur().get_name()
					.equals(joueurs_jeu.getInstance().get_current_joueur().get_name()) == false){
				Attaque attaque = new Attaque(fen,1,0);
			}
			else{
			JOptionPane.showMessageDialog(fen,
