...
public class button2_0_clicked implements ActionListener { 
	
	MainWindow fen;
	
	public button2_0_clicked(MainWindow fenetre){
	fen = fenetre;
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(fen.get_mode()){
			if(Map.getInstance().get_case(2, 0).get_joueur() == null ||
					Map.getInstance().get_case(2, 0).get_joueur().get_name()
			...
			if(Map.getInstance().get_case(2, 0).get_joueur() == null || 
					Map.getInstance().get_case(2, 0).get_joueur().get_name()
					.equals(joueurs_jeu.getInstance().get_current_joueur().get_name()) == false){
				Attaque attaque = new Attaque(fen,2,0);
			}
			else{
			JOptionPane.showMessageDialog(fen,
